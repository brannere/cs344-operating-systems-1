 ...
 *
 * Name: Erick Branner  
 * Email: brannere@oregonstate.edu
 */
...
struct dynarray* dynarray_create() {
  struct dynarray* tmp = malloc(sizeof(struct dynarray));
  
  void** array = (void*)malloc(2*sizeof(int));
  tmp->size = 0;
  tmp->capacity = 2;
  tmp->data = array;
...
void dynarray_free(struct dynarray* da) {
  ...
  if(da != NULL){ // the free function will handle when allocated memory is null
    if(da->data != NULL){
      free(da->data); // free allocated memory inside of da first
    }
    free(da); // now free da
  }
...
void dynarray_insert(struct dynarray* da, void* val) {
  ...
  if((da->size+1) > (da->capacity-1)){
    // copy array to new array TWICE the capacity and +1 the size
    // free old array
    // point to new array
    da->capacity = (da->capacity*2); // Double capacity
    void** new_array = (void*)malloc((2*da->capacity)*sizeof(int)); // Allocate sapce for array twice the size
    for(i = 0; i < da->size; i++){  //populate array
      new_array[i] = da->data[i]; // copy info
    }

    free(da->data); // free the old array
    da->data = new_array; // assign the new array
    da->size = da->size + 1;  //copy data
    da->data[(da->size) - 1] = val; //copy data


  }
  else{
    da->size = da->size +1; // size would now be 1 from an empty array
    da->data[(da->size) - 1] = val; // add new value
  }
...
void dynarray_remove(struct dynarray* da, int idx) {
  ...
  if(idx > da->size){ // error handling
    return;
  }
  else if(idx < 0){ // error handlings
    return;
  }
  else{

    for(i = idx; i < da->size; i++){ // shift array back one index, size--
