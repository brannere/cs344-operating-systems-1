...
struct list* list_create() {
  ...
  // tmp->head->val = 0; // so we dont need to do that...
  // tmp->head->next = malloc(sizeof(struct link));
  // tmp->head->next = NULL;
  // tmp->head->next;

  //We cant assign its value to 0, because it is a void pointer...?
...
void list_insert(struct list* list, void* val) {
  ...
  //this is pushing front
  struct link* old_first = (struct link*)malloc(sizeof(struct link));
  // struct link* old_first;
  old_first->val = list->head->val;
  old_first->next = list->head->next;
  // old_first = list->head;
  // old_first->next = list->head->next;
  // free()
...
void list_remove(struct list* list, void* val, int (*cmp)(void* a, void* b)) {
  ...
  // remove ONLY the first instance of that value
  struct link* curr = list->head; // for position/index
  struct link* prev = list->head;
  struct link* case1 = list->head;
  int counter = 0;
  ...
  // tmp = ptr1->next;
  // tmp->next = ptr1->next->next;

//   return;
// }
  // while((cmp(ptr1->val, val)) != 0){  //reuse for position function

  //   ptr1=ptr1->next;
  //   counter++;
  // }

  // printf("Removing index %d", counter, "\n");

  // Check again
  // if((cmp(ptr1->val, val)) == 0){
  //   tmp->next = ptr1; // Keep track of this value we're removing    
  // }
...
int list_position(struct list* list, void* val, int (*cmp)(void* a, void* b)) {
  ...
  while(tmp != NULL){
    ...
    // printf("while \n");
    if(counter >= list->length -1){ // Added the length member
      return -1;
    }
    else if(cmp(tmp->val, val) == 0){
      return counter;
    }
    else{
      // printf("keep looping \n");
...
void list_reverse(struct list* list) {
  ...
  struct link* tmp = list->head->next;
